...
(define-key global-map "\C-w>" 'enlarge-window-horizontally)
(define-key global-map "\C-w<" 'shrink-window-horizontally)
(define-key global-map "\C-w<" 'shrink-window-horizontally)
(define-key global-map "\C-w+" 'enlarge-window)
(define-key global-map "\C-w-" 'shrink-window)
...
(defun viper-toggle-visual-mode (arg)
  "Toggle visual mode on and off. 
This assumes that transient-mark-mode is active. viper-visual-minor-mode is
also toggled on and off, which will enable the visual mode keymap."
  (interactive "P")
